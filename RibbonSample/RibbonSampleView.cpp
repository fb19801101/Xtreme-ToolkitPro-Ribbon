...
BEGIN_MESSAGE_MAP(CRibbonSampleView, CScrollRichEditView)
	...
	ON_COMMAND(ID_FILE_PRINT, CScrollRichEditView::OnFilePrint)
	ON_COMMAND(ID_FILE_PRINT_DIRECT, CScrollRichEditView::OnFilePrint)
	...
	ON_UPDATE_COMMAND_UI(ID_NEWPAGE_SELECTION, CScrollRichEditView::OnUpdateNeedSel)
	...
	ON_COMMAND(ID_CHAR_BOLD, CScrollRichEditView::OnCharBold)
	ON_COMMAND(ID_CHAR_UNDERLINE, CScrollRichEditView::OnCharUnderline)
	ON_COMMAND(ID_CHAR_ITALIC, CScrollRichEditView::OnCharItalic)
	ON_UPDATE_COMMAND_UI(ID_CHAR_BOLD, CScrollRichEditView::OnUpdateCharBold)
	ON_UPDATE_COMMAND_UI(ID_CHAR_UNDERLINE, CScrollRichEditView::OnUpdateCharUnderline)
	ON_UPDATE_COMMAND_UI(ID_CHAR_ITALIC, CScrollRichEditView::OnUpdateCharItalic)
	...
	ON_COMMAND(ID_INSERT_BULLET, CScrollRichEditView::OnBullet)
	ON_UPDATE_COMMAND_UI(ID_INSERT_BULLET, CScrollRichEditView::OnUpdateBullet)
	...
	ON_COMMAND(ID_TEXT_INSERTOBJECT, CScrollRichEditView::OnInsertObject)
...
{
	// default preparation
	BOOL bPreparePrinting = DoPreparePrinting(pInfo);
	...
	if (bPreparePrinting)
	{
		...
		long lLineWidth = ::MulDiv(dc.GetDeviceCaps(PHYSICALWIDTH), 1440, dc.GetDeviceCaps(LOGPIXELSX));
		GetRichEditCtrl().SetTargetDevice(dc, lLineWidth);

		long lTextLength = GetRichEditCtrl().GetTextLength();
		long lLast  = 0;
		UINT nPages = 0;

		while (lLast < lTextLength)
		{
			fmtRange.chrg.cpMin = lLast;
			fmtRange.chrg.cpMax = -1L;

			lLast = GetRichEditCtrl().FormatRange(&fmtRange, FALSE);
			++nPages;

			if (lLast < fmtRange.chrg.cpMin)
			{
				break;
			}
		}
	...
	return bPreparePrinting;
}

void CRibbonSampleView::OnPrint(CDC *pDC, CPrintInfo *pInfo)
{
	CScrollRichEditView::OnPrint(pDC, pInfo);
}
...
{
	CScrollRichEditView::AssertValid();
}

void CRibbonSampleView::Dump(CDumpContext& dc) const
{
	CScrollRichEditView::Dump(dc);
}
...
void CRibbonSampleView::OnIndentLeftSpin(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (pControl)
	{
		SetIndentLeft(int(StringToDouble(pControl->GetEditText()) * 1000) + tagNMCONTROL->iDelta * 100);
...
void CRibbonSampleView::OnIndentRightSpin(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	if (pControl)
	{
		SetIndentRight(int(StringToDouble(pControl->GetEditText()) * 1000) + tagNMCONTROL->iDelta * 100);
...
void CRibbonSampleView::OnUpdateEditUndo(CCmdUI* pCmd)
{
	...
	{
		CScrollRichEditView::OnUpdateEditUndo(pCmd);
	}
...
void CRibbonSampleView::OnEditUndo(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	{
		CScrollRichEditView::OnEditUndo();
	}
