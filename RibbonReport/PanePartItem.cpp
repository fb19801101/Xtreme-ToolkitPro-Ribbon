...
void CPanePartItem::OnDblclk(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	CString strFile = strPath+strBridge+ _T("\\")+strSteel+ _T("钢筋数量计算表.xlsx");

	CXTPExcelUtil excel;
	excel.InitExcel();
	if(excel.CreateExcel(strSteel+_T("钢筋数量计算表.xlsx"), strPath+strBridge, _T("钢筋数量模板.xlsx"), strPath))
	{
	...
	{
		msgWrg(_T("创建")+strSteel+_T("钢筋数量计算表.xlsx")+_T("失败！"));
	}
...
void CPanePartItem::InitPartItemFromJson()
{
	...
	CStdioFile _file;
	if(_file.Open(strPath+_T("\\项目树结构.csv"), CFile::modeCreate | CFile::modeReadWrite))
	{
		setlocale(LC_ALL,"");
		_file.Seek(CFile::begin,0);

		_file.WriteString(_T("节点名称,节点ID,根节点ID,节点级数,系统节点\n"));
