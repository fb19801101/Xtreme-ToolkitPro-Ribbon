// PanePartItemSys.cpp: implementation of the CPanePartItemSys class.
//
//////////////////////////////////////////////////////////////////////
...
CPanePartItemSys::CPanePartItemSys()
{
...
BEGIN_MESSAGE_MAP(CPanePartItemSys, CPaneHolder)
	//{{AFX_MSG_MAP(CPanePartItemSys)
	ON_WM_CREATE()
	ON_WM_SIZE()
	ON_WM_SETFOCUS()
	//}}AFX_MSG_MAP
...
int CPanePartItemSys::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CPanePartItemSys::OnSize(UINT nType, int cx, int cy)
{
...
void CPanePartItemSys::OnSetFocus(CWnd*)
{
	m_wndTreeCtrl.SetFocus();
}

void CPanePartItemSys::OnTreeSelChange(NMHDR* pNMHDR, LRESULT*)
{  
	NMTREEVIEW* pNMTreeView = (NMTREEVIEW *)pNMHDR;
	if (!pNMTreeView->itemNew.hItem) return;
}

void CPanePartItemSys::OnClick(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	sql.Format(_T("where 节点ID between %d and %d"),id,id+iCount-1);
	pMaainFrame->m_pRibbonReportView->SetView(_T("tv_sys_contract"));
	pMaainFrame->m_pRibbonReportView->DataBindRecordset(GetRecordset(_T("tv_sys_contract"),_T(""),sql));
	int col[] = {0,1};
	xtp_vector<int> cols(col);
	pMaainFrame->m_pRibbonReportView->SetColumnVisible(cols);
	pMaainFrame->m_pRibbonReportView->SetRecordItemEditable(2);

	sql = _T("清单编码");
	pMaainFrame->m_pRibbonReportView->SetRecordItemControl(2, XTP_REPORT_ITEM_CTRL_LINK,_T("tv_sys_guidance"),_T("清单编码,定额编码,指导价编码,计量标识,项目名称,计量单位,单位比率,含税指导单价,含税项目单价"),sql);

	*pResult = 0;
}

void CPanePartItemSys::OnDblclk(NMHDR* pNMHDR, LRESULT* pResult) 
{
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
	if (!hItem) return;
	...
	int iCount = m_wndTreeCtrl.GetSubItemCount(hItem, xtpParamSubTree);
	sql.Format(_T("where 节点ID between %d and %d"),id,id+iCount-1);
	pMaainFrame->m_pRibbonReportView->SetView(_T("tv_sys_quantity_sum"));
	pMaainFrame->m_pRibbonReportView->DataBindRecordset(GetRecordset(_T("tv_sys_quantity_sum"),_T("数量ID,劳务名称,清单编码,施工日期,数量名称,计量单位,分项概算数量,图纸设计数量,图纸变更数量,复核图纸设计数量,复核图纸变更数量,现场设计数量,现场变更数量,已完设计数量,已完变更数量,对上计价设计数量,对上计价变更数量,对下计价设计数量,对下计价变更数量"),sql));

	*pResult = 0;
}

void CPanePartItemSys::RefreshItem()
{
	CMainFrame* pMaainFrame = (CMainFrame*)AfxGetMainWnd();
...
CString CPanePartItemSys::GetCategoryName(const CString& str)
{
	if (str == _T("(None)"))
		return _T("");
...
CObject* CPanePartItemSys::RefreshPropertyGrid(CXTPPropertyGrid* pPropertyGrid) 
{
	HTREEITEM hItem = m_wndTreeCtrl.GetSelectedItem();
...
BOOL CPanePartItemSys::OnPropertyGridValueChanged(CObject* pActiveObject, CXTPPropertyGridItem* pItem)
{
...
void CPanePartItemSys::OnRibbonNew()
{
	...
	if (!hItemRibbon) hItemRibbon = m_wndTreeCtrl.InsertItem(_T("PartItemSysList"), 0, 0);

	m_strFileName.Empty();
}

void CPanePartItemSys::InitPartTypeActions()
{
	m_wndTreeCtrl.DeleteAllItems();

	HTREEITEM hItemPartItem = m_wndTreeCtrl.GetRootItem();
	if (!hItemPartItem) hItemPartItem = m_wndTreeCtrl.InsertItem(_T("PartItemSysList"),0,0);
	m_wndTreeCtrl.SetItemColor(hItemPartItem,RGB(255,0,0));

	CString sql;
	sql.Format(_T("select spi_level, spi_node, spi_id, spi_rootid from %s[tb_sys_partitem] \
				  group by spi_level, spi_node, spi_id, spi_rootid \
				  order by spi_id"),g_TablePrix);
	...
	while (!rst.IsEOF())
	{
		...
		if(tvi.pid == 0)
		{
			strCategory = _T("PartItemSysList");
...
void CPanePartItemSys::OnRibbonOpen()
{
	m_wndTreeCtrl.OpenTreeFile(xtpFileFormatBinary);
}

void CPanePartItemSys::OnRibbonSave()
{
	m_wndTreeCtrl.SaveTreeFile(xtpFileFormatBinary);
}

void CPanePartItemSys::OnRibbonExpand()
{
	m_wndTreeCtrl.ExpandItems();
	m_bTreeExpand = TRUE;
}

void CPanePartItemSys::OnRibbonCollapse()
{
	m_wndTreeCtrl.CollapseItems();
	m_bTreeExpand = FALSE;
}

void CPanePartItemSys::OnRibbonInit()
{
	BOOL ret = msgAsk(_T("是否初始化分部分项树形结构？"));
	if(ret)
	{
		InitPartTypeActions();
		msgInf(_T("初始化分部分项树形结构成功！"));
	}
}

void CPanePartItemSys::OnUpdateRibbonExpand(CCmdUI* pCmdUI)
{
...
void CPanePartItemSys::OnUpdateRibbonCollapse(CCmdUI* pCmdUI)
{
	CXTPControl* pControl = CXTPControl::FromUI(pCmdUI);
