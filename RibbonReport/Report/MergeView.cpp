...
BEGIN_MESSAGE_MAP(CMergeView, CXTPReportView)
	...
	ON_COMMAND(ID_REPORT_COLUMN_AUTOSIZING,    OnReportAutoSizing)
	...
	//
	ON_UPDATE_COMMAND_UI(ID_REPORT_COLUMN_AUTOSIZING,    OnUpdateReportAutoSizing)
...
int CMergeView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	wndReport.GetImageManager()->SetIcons(IDB_TASKLIST, 0, 0, CSize(16, 16));
...
#if _XTPLIB_VERSION_PREFIX >= 1501
	wndReport.GetHeaderRecords()->MergeItems(CXTPReportRecordItemRange(3,4,1,2));
	wndReport.GetFooterRecords()->MergeItems(CXTPReportRecordItemRange(2,4,3,4));
