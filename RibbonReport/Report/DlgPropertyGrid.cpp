// DlgPropertyGrid.cpp : implementation file
//
...
/////////////////////////////////////////////////////////////////////////////
// CDlgPropertyGrid dialog

CDlgPropertyGrid::CDlgPropertyGrid(CWnd* pParent /*=NULL*/)
	: CXTPResizeDialog(CDlgPropertyGrid::IDD, pParent)
	, m_bHelpVisible(TRUE)
...
void CDlgPropertyGrid::DoDataExchange(CDataExchange* pDX)
{
	CXTPResizeDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlgPropertyGrid)
	DDX_Control(pDX, IDC_PROPERTYGRID_CB_THEME, m_cmbTheme);
	DDX_Control(pDX, IDC_PROPERTYGRID_CB_BORDER, m_cmbBorder);
	DDX_Control(pDX, IDC_PROPERTYGRID_SC_PLACEHOLDER, m_wndPlaceHolder);
	DDX_CBIndex(pDX, IDC_PROPERTYGRID_CB_THEME, m_nTheme);
	DDX_Check(pDX, IDC_PROPERTYGRID_CK_TABITEMS, m_bTabItems);
	//}}AFX_DATA_MAP
	DDX_Check(pDX, IDC_PROPERTYGRID_CK_HELP, m_bHelpVisible);
	DDX_Check(pDX, IDC_PROPERTYGRID_CK_VERBS, m_bVerbsVisible);
	DDX_Check(pDX, IDC_PROPERTYGRID_CK_TOOLBAR, m_bToolBarVisible);
	DDX_Check(pDX, IDC_PROPERTYGRID_CK_DOUBLE, m_bDoubleFont);
	DDX_Check(pDX, IDC_PROPERTYGRID_CK_MARKUP, m_bMarkup);
	DDX_Radio(pDX, IDC_PROPERTYGRID_BN_SORT_CATEGORIES, m_nSort);
	DDX_Check(pDX, IDC_PROPERTYGRID_CK_CUSTOMCOLORS, m_bCustomColors);
	DDX_Check(pDX, IDC_PROPERTYGRID_CK_HIGHLIGHT, m_bHighlightChanged);
	DDX_Check(pDX, IDC_PROPERTYGRID_CK_SHOWBUTTONS, m_bShowButtons);
	DDX_Check(pDX, IDC_PROPERTYGRID_CK_RIGHTTOLEFT, m_bRightToLeft);
	
}

BEGIN_MESSAGE_MAP(CDlgPropertyGrid, CXTPResizeDialog)
	//{{AFX_MSG_MAP(CDlgPropertyGrid)
	ON_BN_CLICKED(IDC_PROPERTYGRID_BN_SWITCHSTATE, OnButtonSwitchstate)
	ON_CBN_SELCHANGE(IDC_PROPERTYGRID_CB_THEME, OnSelchangeComboTheme)
	ON_CBN_SELCHANGE(IDC_PROPERTYGRID_CB_BORDER, OnSelchangeComboBorder)
	ON_BN_CLICKED(IDC_PROPERTYGRID_CK_TABITEMS, OnChkTabitems)
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_PROPERTYGRID_CK_TOOLBAR, OnClickedChkToolbar)
	ON_BN_CLICKED(IDC_PROPERTYGRID_CK_HELP, OnClickedChkHelp)
	ON_BN_CLICKED(IDC_PROPERTYGRID_CK_VERBS, OnClickedChkVerbs)
	ON_BN_CLICKED(IDC_PROPERTYGRID_CK_DOUBLE, OnClickedChkDouble)
	ON_BN_CLICKED(IDC_PROPERTYGRID_CK_MARKUP, OnClickedChkMarkup)
	ON_BN_CLICKED(IDC_PROPERTYGRID_CK_HIGHLIGHT, OnClickedHighlightChanged)
	ON_BN_CLICKED(IDC_PROPERTYGRID_BN_SORT_CATEGORIES, OnClickedSort)
	ON_BN_CLICKED(IDC_PROPERTYGRID_BN_SORT_ALPHABETICAL, OnClickedSort)
	ON_BN_CLICKED(IDC_PROPERTYGRID_BN_SORT_NOSORT, OnClickedSort)
	ON_BN_CLICKED(IDC_PROPERTYGRID_CK_SHOWBUTTONS, OnClickShowButtons)
	ON_BN_CLICKED(IDC_PROPERTYGRID_CK_RIGHTTOLEFT, OnClickRightToLeft)
	ON_MESSAGE(XTPWM_PROPERTYGRID_NOTIFY, OnGridNotify)
	ON_BN_CLICKED(IDC_PROPERTYGRID_CK_CUSTOMCOLORS, OnClickedCustomcolors)
	ON_BN_CLICKED(IDC_PROPERTYGRID_BN_ADD, OnClickedAdd)
	ON_BN_CLICKED(IDC_PROPERTYGRID_BN_SET, OnClickedSet)
	ON_BN_CLICKED(IDC_PROPERTYGRID_BN_DEL, OnClickedDel)

END_MESSAGE_MAP()

/////////////////////////////////////////////////////////////////////////////
// CDlgPropertyGrid message handlers

BOOL CDlgPropertyGrid::OnInitDialog()
{
	...
	if ( m_wndPropertyGrid.Create( rc, this, IDC_PROPERTYGRID_CONTROL ) )
	{
		...
		pItemSaveOnClose->SetID(ID_GRID_ITEM_SAVE_ON_CLOSE);
		...
		CXTPPropertyGridItem* pItemLanguage = pVersion->AddChildItem(new CXTPPropertyGridItem(ID_GRID_ITEM_VERSION_LANGUAGE, _T("English (United States)")));
	...
	//////////////////////////////////////////////////////////////////////////
		...
		UINT btnFilter[] = {100};
		m_wndPropertyGrid.GetImageManager()->SetIcons(IDB_FILTER, btnFilter, 1, 0);
	...
	//////////////////////////////////////////////////////////////////////////
		...
		m_wndPropertyGrid.GetImageManager()->SetIcons(IDB_CONSTRAINTS, 0, 5, CSize(20, 14));
		...
		// add child items to category.
		CXTPPropertyGridItem* pItemIcon = pCustom->AddChildItem(new CPropertyItemIcon(_T("Icon"), m_hIcon));
		CXTPPropertyGridItem* pItemDock = pCustom->AddChildItem(new CPropertyItemChilds(_T("DockPadding"), CRect(100, 20, 400, 50)));
		pCustom->AddChildItem(new CPropertyItemColor(_T("CustomCombolist"), RGB(0xFF, 0x80, 0x40)));
		...
		pCustom->AddChildItem(new CPropertyItemFileBox(_T("File Box")));
		...
		pCustom->AddChildItem(new CPropertyItemUpperCase(_T("UpperCase")));

		pCustom->AddChildItem(new CPropertyItemIPAddress(_T("IP Address")));		

		pCustom->AddChildItem(new CPropertyItemMenu(_T("Popup Menu")));
	
		pCustom->AddChildItem(new CPropertyItemEdit(_T("Output"), _T("Debug")));
		...
		CXTPPropertyGridItem* pItemSpin = pCustom->AddChildItem(new CPropertyItemSpin(_T("SpinButton")));
		pItemSpin->SetDescription(_T("This sample shows how to add new button type"));

		pCustom->AddChildItem(new CPropertyItemSlider(_T("Slider")));
		
		CPropertyItemCheckBox* pItemCheckBox = (CPropertyItemCheckBox*)pCustom->AddChildItem(new CPropertyItemCheckBox(_T("Check Box")));
		pItemCheckBox->SetValue(_T("agree with conditions"));
		pItemCheckBox->SetBool(TRUE);

		pCustom->AddChildItem(new CPropertyItemButton(_T("Left Origin"), FALSE, TRUE));
		pCustom->AddChildItem(new CPropertyItemButton(_T("Right Origin"), FALSE, TRUE));
		pCustom->AddChildItem(new CPropertyItemButton(_T("Pointer"), TRUE, TRUE));
		pCustom->AddChildItem(new CPropertyItemButton(_T("Gradient"), TRUE, FALSE));
	}


	m_groupAppearance.SubclassDlgItem(IDC_PROPERTYGRID_GP_APPEAR, this);
	m_groupSort.SubclassDlgItem(IDC_PROPERTYGRID_GP_SORT, this);
	m_groupColor.SubclassDlgItem(IDC_PROPERTYGRID_GP_COLOR, this);

	// Set control resizing.
	SetResize(IDC_PROPERTYGRID_CONTROL, XTP_ANCHOR_TOPLEFT, XTP_ANCHOR_BOTTOMRIGHT);

	SetResize(IDC_PROPERTYGRID_GP_APPEAR,       XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_CK_TOOLBAR,       XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_CK_HELP,          XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_CK_VERBS,         XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_CK_DOUBLE,        XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_CK_TABITEMS,      XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_CK_HIGHLIGHT,     XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_CB_THEME,       XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_GP_SORT,         XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_BN_SORT_CATEGORIES,   XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_BN_SORT_ALPHABETICAL, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_BN_SORT_NOSORT,       XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_GP_COLOR,        XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_CK_CUSTOMCOLORS,  XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_CK_MARKUP,        XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_BN_SWITCHSTATE,  XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_CB_BORDER,      XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_CK_SHOWBUTTONS, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_CK_RIGHTTOLEFT, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_BN_ADD, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_BN_SET, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_PROPERTYGRID_BN_DEL, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	...
	m_cmbTheme.AddString(_T("xtpGridThemeVisualStudio2010"));
	m_cmbTheme.SetCurSel(1);
...
void CDlgPropertyGrid::OnClickedChkToolbar()
{
	UpdateData();
	m_wndPropertyGrid.ShowToolBar(m_bToolBarVisible);
}

void CDlgPropertyGrid::OnClickedChkHelp()
{
	UpdateData();
	m_wndPropertyGrid.ShowHelp(m_bHelpVisible);
}

void CDlgPropertyGrid::OnClickedChkVerbs()
{
	UpdateData();
	if (m_bVerbsVisible)
	{
...
void CDlgPropertyGrid::OnClickedSort()
{
	UpdateData();
	m_wndPropertyGrid.SetPropertySort((XTPPropertyGridSortOrder)m_nSort);
}

LRESULT CDlgPropertyGrid::OnGridNotify(WPARAM wParam, LPARAM lParam)
{
	...
	if (wParam == XTP_PGN_SELECTION_CHANGED)
	{
		...
		if (pItem)
		{
			...
			if (pItem->GetID() == ID_GRID_ITEM_VERSION_LANGUAGE)
			{
	...
	if (wParam == XTP_PGN_EDIT_CHANGED)
	{
		...
		{
			// Custom Validation
			if (pEdit->GetItem()->GetID() == ID_GRID_ITEM_VERSION_LANGUAGE)
			{
				CString str;
...
void CDlgPropertyGrid::OnClickedCustomcolors()
{
	UpdateData();
...
void CDlgPropertyGrid::OnOK()
{
	CXTPPropertyGridItem* pItem = 0;

	pItem = m_wndPropertyGrid.FindItem(ID_GRID_ITEM_VERSION_LANGUAGE);
...
void CDlgPropertyGrid::OnButtonSwitchstate()
{
	if (m_bCollapse)
	{
...
void CDlgPropertyGrid::OnClickedChkDouble()
{
	UpdateData();
...
void CDlgPropertyGrid::OnClickedChkMarkup()
{
	UpdateData();

	m_wndPropertyGrid.EnableMarkup(m_bMarkup);

	CXTPPropertyGridItem *pItemSaveOnClose = m_wndPropertyGrid.FindItem(ID_GRID_ITEM_SAVE_ON_CLOSE);
	CXTPPropertyGridItem *pItemLanguage	   = m_wndPropertyGrid.FindItem(ID_GRID_ITEM_VERSION_LANGUAGE);

	ASSERT(pItemSaveOnClose);
	ASSERT(pItemLanguage);

	if (m_bMarkup)
	{
		if (pItemSaveOnClose)
		{
			pItemSaveOnClose->SetCaption(_T("<TextBlock VerticalAlignment='Center'><Span Foreground=\"blue\">SaveOnClose</Span></TextBlock>"));
			pItemSaveOnClose->SetMarkupTemplate(_T("<TextBlock VerticalAlignment='Center' TextWrapping='Wrap'>It's <Bold>%value%!</Bold> <Underline>Markup</Underline> <Span Foreground=\"red\">works</Span>!</TextBlock>"));
		}

		if (pItemLanguage)
		{
			pItemLanguage->SetMarkupTemplate(_T("<TextBlock VerticalAlignment='Center'><Bold><Span Foreground=\"red\">%value%</Span></Bold></TextBlock>"));
		}
	}
	else
	{
		if (pItemSaveOnClose)
		{
			pItemSaveOnClose->SetCaption(_T("SaveOnClose"));
			pItemSaveOnClose->SetMarkupTemplate(_T(""));
		}

		if (pItemLanguage)
		{
			pItemLanguage->SetMarkupTemplate(_T(""));
		}
	}
}

void CDlgPropertyGrid::OnSelchangeComboTheme()
{
	UpdateData();
	m_wndPropertyGrid.SetTheme((XTPPropertyGridPaintTheme)m_nTheme);
}

void CDlgPropertyGrid::OnSelchangeComboBorder()
{
	UpdateData();
	m_wndPropertyGrid.SetBorderStyle((XTPPropertyGridBorderStyle)m_cmbBorder.GetCurSel());
}

void CDlgPropertyGrid::OnChkTabitems()
{
	UpdateData();
	m_wndPropertyGrid.NavigateItems(m_bTabItems);
}

void CDlgPropertyGrid::OnClickedHighlightChanged()
{
	UpdateData();
	m_wndPropertyGrid.HighlightChangedItems(m_bHighlightChanged);
}

void CDlgPropertyGrid::OnClickShowButtons()
{
	UpdateData();
...
void CDlgPropertyGrid::OnClickRightToLeft()
{
	UpdateData();
...
void CDlgPropertyGrid::OnClickedAdd()
{
	UpdateData();
	m_wndPropertyGrid.SetLayoutRTL(m_bRightToLeft);
}

void CDlgPropertyGrid::OnClickedSet()
{
	UpdateData();
	m_wndPropertyGrid.SetLayoutRTL(m_bRightToLeft);
}

void CDlgPropertyGrid::OnClickedDel()
{
	UpdateData();
	m_wndPropertyGrid.SetLayoutRTL(m_bRightToLeft);
}
