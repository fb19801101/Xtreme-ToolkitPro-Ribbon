...
void CCreditStatic::SetCredits(CXTPString credits,char delimiter)
{
	char *str,*ptr1,*ptr2;
...
{
	if(index <= NORMAL_TEXT_COLOR) return;
...
{
	if(index <= NORMAL_TEXT_HEIGHT) return;
...
{
	if(index <= DISPLAY_BITMAP) return;
...
{
	if(value <= GRADIENT_LEFT_LIGHT) return;
...
//			bUnderline = TRUE;
	...
	if (!m_bProcessingBitmap)
	{
		...
		int x = memDC.DrawText(m_szWork,m_szWork.GetLength()-rmcode,&r,DT_TOP|DT_CENTER|
