...
CHyperLink::CHyperLink()
{
    ...
    m_crHoverColour     = RGB(255, 92, 0);      //GetSysColor(COLOR_3DHILIGHT);
...
{
    if(nCtlColor == CTLCOLOR_STATIC) return (HBRUSH)GetStockObject(NULL_BRUSH);
    UNUSED(nCtlColor);
...
void CHyperLink::PreSubclassWindow() 
{
    ...
    if (strWndText.IsEmpty()) {
        if(m_strURL.IsEmpty()) return;    // Window and URL both NULL. DUH!
    ...
    if ((UINT)result <= HINSTANCE_ERROR) {
        ...
        if (GetRegKey(HKEY_CLASSES_ROOT, _T(".htm"), key) == ERROR_SUCCESS) {
            ...
            if (GetRegKey(HKEY_CLASSES_ROOT,key,key) == ERROR_SUCCESS) {
                ...
                result = (HINSTANCE) WinExec(CXTPString(key),showcmd);
            }
        }
    }
