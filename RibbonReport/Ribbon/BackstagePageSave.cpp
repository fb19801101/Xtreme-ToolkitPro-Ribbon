...
CBackstagePageSave::CBackstagePageSave() : CXTPRibbonBackstagePage(CBackstagePageSave::IDD)
{
	...
	UINT nIDs[] = {IDC_SAVE_BN_SEND_EMAIL, IDC_SAVE_BN_SAVE_WEB, IDC_SAVE_BN_SAVE_SHAREPOINT, 
		IDC_SAVE_BN_PUBLISH, IDC_SAVE_BN_CHANGEFILETYPES, IDC_SAVE_BN_CREATE_PDF};
...
void CBackstagePageSave::DoDataExchange(CDataExchange* pDX)
{
	...
	//}}AFX_DATA_MAP
	DDX_Control(pDX, IDC_SAVE_SC_SEPARATOR_1, m_lblSeparator1);
	DDX_Control(pDX, IDC_SAVE_SC_SEPARATOR_2, m_lblSeparator2);
	DDX_Control(pDX, IDC_SAVE_SC_SEPARATOR_3, m_lblSeparator4);

	DDX_Control(pDX, IDC_SAVE_TXT_CAPTION, m_lblSaveAndSend);
	DDX_Control(pDX, IDC_SAVE_TXT_FILETYPES, m_lblFileTypes);

	DDX_Control(pDX, IDC_SAVE_BN_SEND_EMAIL, m_btnSendEmail);
	DDX_Control(pDX, IDC_SAVE_BN_SAVE_WEB, m_btnSaveToWeb);
	DDX_Control(pDX, IDC_SAVE_BN_SAVE_SHAREPOINT, m_btnSaveToSharePoint);
	DDX_Control(pDX, IDC_SAVE_BN_PUBLISH, m_btnPublish);
	DDX_Control(pDX, IDC_SAVE_BN_CHANGEFILETYPES, m_btnChangeFileType);
	DDX_Control(pDX, IDC_SAVE_BN_CREATE_PDF, m_btnCreatePDF);
}
...
BEGIN_MESSAGE_MAP(CBackstagePageSave, CXTPRibbonBackstagePage)
	...
	//}}AFX_MSG_MAP
	ON_BN_SETFOCUS(IDC_SAVE_BN_SEND_EMAIL, OnButtonSelected)
	ON_BN_SETFOCUS(IDC_SAVE_BN_SAVE_WEB, OnButtonSelected)
	ON_BN_SETFOCUS(IDC_SAVE_BN_SAVE_SHAREPOINT, OnButtonSelected)
	ON_BN_SETFOCUS(IDC_SAVE_BN_PUBLISH, OnButtonSelected)
	ON_BN_SETFOCUS(IDC_SAVE_BN_CHANGEFILETYPES, OnButtonSelected)
	ON_BN_SETFOCUS(IDC_SAVE_BN_CREATE_PDF, OnButtonSelected)
END_MESSAGE_MAP()
...
BOOL CBackstagePageSave::OnInitDialog() 
{
	...
	SetResize(IDC_SAVE_SC_SEPARATOR_3, XTP_ANCHOR_TOPLEFT, XTP_ANCHOR_BOTTOMLEFT);


	CXTPWindowRect rcPage(GetDlgItem(IDC_SAVE_SC_PAGE));
