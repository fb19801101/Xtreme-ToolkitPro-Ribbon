...
void CCustomizePageGeneral::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Check(pDX, IDC_GENERAL_CK_SHOWEYBOARDTIPS, m_bShowKeyboardTips);
	DDX_Check(pDX, IDC_GENERAL_CK_SHOWMINITOOLBAR, m_bShowMiniToolbar);
	DDX_Control(pDX, IDC_GENERAL_TXT_CAPTION, m_wndCaption1);
	DDX_Control(pDX, IDC_GENERAL_TXT_TITLE, m_wndTitle);
	DDX_CBIndex(pDX, IDC_GENERAL_CB_COLOR, m_nColor);
	//}}AFX_DATA_MAP
}
...
BOOL CCustomizePageGeneral::OnInitDialog() 
{
	...
	SetResize(IDC_GENERAL_TXT_CAPTION, XTP_ANCHOR_TOPLEFT, XTP_ANCHOR_TOPRIGHT);

	m_wndTitle.SetMarkupTextEx(_T("<StackPanel Orientation='Horizontal'><Image Source='res://IDP_OPTIONSICON_GENERAL'/>")
	...
	m_nColor = pMainFrame->m_idRibbonStyle - ID_THEME_STYLE_BLUE2007;
...
void CCustomizePageGeneral::OnOK() 
{
	...
	if (m_nColor != (int)pMainFrame->m_idRibbonStyle - ID_THEME_STYLE_BLUE2007)
	{
		pMainFrame->OnThemeStylePopup(m_nColor + ID_THEME_STYLE_BLUE2007);
	}
