...
CGalleryItemVariant::CGalleryItemVariant(int nId, CXTPImageManagerIcon* pImage)
{
	CString str = LoadResourceString(nId);
	m_nId = nId;
	...
	if(nIndex > -1)
	{
		m_strCaption = str.Mid(0,nIndex);
		m_strDescriptionText = str.Mid(nIndex+1,nCount-nIndex-1);
	}

	SetData((DWORD_PTR)pImage);
}

CGalleryItemVariant::CGalleryItemVariant(LPCTSTR lpszCaption, CXTPImageManagerIcon* pImage)
{
...
CGalleryItemVariant::CGalleryItemVariant(COleVariant vCaption, CXTPImageManagerIcon* pImage)
{
	CString str;
	switch (vCaption.vt)
	{
	case VT_BSTR:
		str = vCaption.bstrVal;
		break;
	case VT_UINT:
		str = LoadResourceString(vCaption.uintVal);
		m_nId = vCaption.uintVal;
		break;
	case VT_INT:
	case VT_I4:
		str = LoadResourceString(vCaption.intVal);
		m_nId = vCaption.intVal;
		break;
	case VT_R8:
		str = ToString(vCaption.dblVal);
		break;
	}
...
CGalleryItemVariant::CGalleryItemVariant(int nId, LPCTSTR lpszCaption, CXTPImageManagerIcon* pImage)
{
	CString str = lpszCaption;
	m_nId = nId;
	int nCount = str.GetLength();
...
CGalleryItemVariant::CGalleryItemVariant(int nId, COleVariant vCaption, CXTPImageManagerIcon* pImage)
{
	CString str;
	switch (vCaption.vt)
	{
...
CGalleryItemVariant::CGalleryItemVariant(CGalleryItemVariant& src)
{
	m_nId = src.m_nId;
	m_strCaption = src.m_strCaption;
	m_strDescriptionText = src.m_strDescriptionText;
	SetData(src.GetData());
}

CGalleryItemVariant::~CGalleryItemVariant()
{

}

CGalleryItemVariant& CGalleryItemVariant::operator= (CGalleryItemVariant& src)
{
	m_nId = src.m_nId;
	m_strCaption = src.m_strCaption;
	m_strDescriptionText = src.m_strDescriptionText;
...
CGalleryItemVariant& CGalleryItemVariant::operator= (int nId)
{
	CString str = LoadResourceString(nId);
	m_nId = nId;
	int nCount = str.GetLength();
...
CGalleryItemVariant& CGalleryItemVariant::operator= (LPCTSTR lpszCaption)
{
	CString str = lpszCaption;
	int nCount = str.GetLength();
	int nIndex = str.ReverseFind('|');
...
CGalleryItemVariant& CGalleryItemVariant::operator= (COleVariant vCaption)
{
	CString str;
	switch (vCaption.vt)
	{
...
CGalleryItemVariant& CGalleryItemVariant::operator= (CXTPImageManagerIcon* pImage)
{
	SetData((DWORD_PTR)pImage);
	m_nId = pImage->GetID();
	CString str = LoadResourceString(m_nId);
	int nCount = str.GetLength();
	int nIndex = str.ReverseFind('|');
	m_strCaption = str;
	if(nIndex > -1)
...
void CGalleryItemVariant::Draw(CDC* pDC, CXTPControlGallery* pGallery, CRect rcItem, BOOL bEnabled, BOOL bSelected, BOOL bPressed, BOOL bChecked)
{
	CXTPPaintManager* pPaintManager = pGallery->GetPaintManager();
	...
	CXTPImageManagerIcon* pImage = (CXTPImageManagerIcon*)GetData();
	if(pImage)
	{
		CSize szImage(pImage->GetWidth(), pImage->GetHeight());
		CPoint pt(rcItem.left + pPaintManager->m_nPopupBarTextPadding + cx, (rcItem.top + rcItem.bottom - szImage.cy) / 2);
		rcText.left += 3*szImage.cx/2;

		if(m_strCaption.IsEmpty())
		{
			pt = CPoint((rcItem.left + rcItem.right)/2-szImage.cx/2, (rcItem.top + rcItem.bottom - szImage.cy) / 2);
		}

		pImage->Draw(pDC, pt, pImage->GetIcon(bEnabled ? xtpImageNormal : xtpImageDisabled), szImage);
	}

	if(!m_strDescriptionText.IsEmpty() && !m_strCaption.IsEmpty())
	{
		LOGFONT lf;
		pPaintManager->GetRegularFont()->GetLogFont(&lf);
		lf.lfHeight += 2;
		CFont ft;
		ft.CreateFontIndirect(&lf);
		CXTPFontDC font(pDC, &ft);

		rcText.left += pPaintManager->m_nPopupBarTextPadding + cx;
		rcText.top = rcText.top+rcItem.Height()/2;
		rcText.bottom = rcText.top+rcItem.Height()/2;
		pDC->DrawText(m_strDescriptionText, rcText, DT_END_ELLIPSIS|DT_SINGLELINE|DT_TOP);

		lf.lfWeight = 700;
		lf.lfHeight -= 4;
		lstrcpy( lf.lfFaceName, _T("ºÚÌå"));
		ft.DeleteObject();
		ft.CreateFontIndirect(&lf);
		font = CXTPFontDC(pDC, &ft);

		rcText.bottom = rcText.top;
		rcText.top = rcText.bottom-rcItem.Height()/2;
		pDC->DrawText(m_strCaption, rcText, DT_END_ELLIPSIS|DT_SINGLELINE|DT_VCENTER);
	}
