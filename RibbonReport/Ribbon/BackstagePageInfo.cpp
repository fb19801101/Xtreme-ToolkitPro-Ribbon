...
void CBackstagePageInfo::DoDataExchange(CDataExchange* pDX)
{
	...
	//}}AFX_DATA_MAP
	DDX_Control(pDX, IDC_INFO_SC_SEPARATOR_1, m_lblSeparator1);
	DDX_Control(pDX, IDC_INFO_SC_SEPARATOR_2, m_lblSeparator2);
	DDX_Control(pDX, IDC_INFO_SC_SEPARATOR_3, m_lblSeparator3);
	DDX_Control(pDX, IDC_INFO_SC_SEPARATOR_4, m_lblSeparator4);

	DDX_Control(pDX, IDC_INFO_TXT_CAPTION, m_lblInformation);

	DDX_Control(pDX, IDC_INFO_TXT_PERMISSIONS, m_lblPermissions);
	DDX_Control(pDX, IDC_INFO_TXT_PREPAREFORSHARING, m_lblPrepareForSharing);
	DDX_Control(pDX, IDC_INFO_TXT_VERSIONS, m_lblVersions);
	
	DDX_Control(pDX, IDC_INFO_BN_PROTECTDOCUMENT, m_btnProtectDocument);
	DDX_Control(pDX, IDC_INFO_CK_ISSUE, m_btnCheckForIssues);
	DDX_Control(pDX, IDC_INFO_BN_MANAGEVERSIONS, m_btnManageVersions);
}


BEGIN_MESSAGE_MAP(CBackstagePageInfo, CXTPRibbonBackstagePage)
	//{{AFX_MSG_MAP(CBackstagePageInfo)
	ON_BN_CLICKED(IDC_INFO_SC_PREVIEW, OnStaticPreview)
	//}}AFX_MSG_MAP
	ON_CBN_DROPDOWN(IDC_INFO_BN_PROTECTDOCUMENT, OnProtectDocumentDropDown)
	ON_CBN_DROPDOWN(IDC_INFO_CK_ISSUE, OnCheckForIssuesDropDown)
...
BOOL CBackstagePageInfo::OnInitDialog() 
{
	...
	InitButton(IDC_INFO_BN_PROTECTDOCUMENT, TRUE);
	InitButton(IDC_INFO_CK_ISSUE, TRUE);
	InitButton(IDC_INFO_BN_MANAGEVERSIONS, FALSE);

	SetResize(IDC_INFO_SC_SEPARATOR_1, XTP_ANCHOR_TOPLEFT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_INFO_SC_SEPARATOR_2, XTP_ANCHOR_TOPLEFT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_INFO_SC_SEPARATOR_3, XTP_ANCHOR_TOPLEFT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_INFO_SC_SEPARATOR_4, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_BOTTOMRIGHT);

	SetResize(IDC_INFO_SC_PREVIEW, XTP_ANCHOR_TOPRIGHT, XTP_ANCHOR_TOPRIGHT);
	

	SetResize(IDC_INFO_TXT_TEXT1, XTP_ANCHOR_TOPLEFT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_INFO_TXT_TEXT2, XTP_ANCHOR_TOPLEFT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_INFO_TXT_TEXT3, XTP_ANCHOR_TOPLEFT, XTP_ANCHOR_TOPRIGHT);
...
void CBackstagePageInfo::OnProtectDocumentDropDown()
{
	...
	XTPImageManager()->SetIcons(IDC_INFO_BN_PROTECTDOCUMENT, nIDs, 1, CSize(0, 0), xtpImageNormal);


	CXTPWindowRect rcButton(GetDlgItem(IDC_INFO_BN_PROTECTDOCUMENT));
...
void CBackstagePageInfo::OnCheckForIssuesDropDown()
{
	...
	CXTPWindowRect rcButton(GetDlgItem(IDC_INFO_CK_ISSUE));	
	pMainFrame->GetCommandBars()->TrackPopupMenu(pPopupBar, TPM_RETURNCMD | TPM_NONOTIFY, rcButton.left, rcButton.bottom, this);
