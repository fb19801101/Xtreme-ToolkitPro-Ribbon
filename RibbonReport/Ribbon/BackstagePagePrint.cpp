...
void CBackstagePagePrint::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_PRINT_ET_COPIES, m_nCopies);
	//}}AFX_DATA_MAP
	DDX_Control(pDX, IDC_PRINT_SC_SEPARATOR_2, m_lblSeparator4);
	DDX_Control(pDX, IDC_PRINT_SC_SEPARATOR_1, m_lblSeparator1);
	DDX_Control(pDX, IDC_PRINT_BN_PRINT, m_btnPrint);

	DDX_Control(pDX, IDC_PRINT_TXT_CAPTION, m_lblPrint);
...
BEGIN_MESSAGE_MAP(CBackstagePagePrint, CXTPRibbonBackstagePage)
	...
	ON_BN_CLICKED(IDC_PRINT_BN_PRINT, OnButtonPrint)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
BOOL CBackstagePagePrint::OnInitDialog() 
{
	...
	InitButton(IDC_PRINT_BN_PRINT);
	m_lblSeparator4.SetVerticalStyle(TRUE);
	
	m_lblPrint.SetFont(&m_fntCaption);
	m_lblPrint.SetTextColor(RGB(94, 94, 94));
	
	SetResize(IDC_PRINT_SC_SEPARATOR_2, XTP_ANCHOR_TOPLEFT, XTP_ANCHOR_BOTTOMLEFT);
	...
	CXTPWindowRect rc(GetDlgItem(IDC_PRINT_SC_PAGE));
	ScreenToClient(rc);
