...
void CBackstagePageRecent::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_RECENT_SC_SEPARATOR, m_lblSeparator1);
	DDX_Control(pDX, IDC_RECENT_TXT_CAPTION, m_lblRecent);

	DDX_Control(pDX, IDC_RECENT_TXT_RECENTFILELIST, m_wndList);
...
BEGIN_MESSAGE_MAP(CBackstagePageRecent, CXTPRibbonBackstagePage)
	...
	ON_BN_CLICKED(IDC_RECENT_TXT_RECENTFILELIST, OnRecentFileListClick)
...
BOOL CBackstagePageRecent::OnInitDialog() 
{
	...
	SetResize(IDC_RECENT_SC_SEPARATOR, XTP_ANCHOR_TOPLEFT, XTP_ANCHOR_TOPRIGHT);
	SetResize(IDC_RECENT_TXT_RECENTFILELIST, XTP_ANCHOR_TOPLEFT, XTP_ANCHOR_BOTTOMRIGHT);
