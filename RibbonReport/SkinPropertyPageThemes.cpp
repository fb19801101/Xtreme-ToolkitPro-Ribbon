...
void CSkinPropertyPageThemes::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_SKIN_LIST_THEMES, m_lboxSkins);
	DDX_Control(pDX, IDC_SKIN_COMBO_FONTS, m_cmbFonts);
	DDX_Control(pDX, IDC_SKIN_COMBO_COLORS, m_cmbColors);
	DDX_Radio(pDX, IDC_SKIN_RADIO_OFFICE2000, m_nTheme);
	DDX_Radio(pDX, IDC_SKIN_RADIO_OFFICE2007, m_nSchema);
	DDX_Radio(pDX, IDC_SKIN_RADIO_SKINLOCAL, m_nLocalSkins);
	DDX_Check(pDX, IDC_SKIN_CHECK_APPLYMETRICS, m_bApplyMetrics);
	DDX_Check(pDX, IDC_SKIN_CHECK_APPLYFRAME, m_bApplyFrame);
	DDX_Check(pDX, IDC_SKIN_CHECK_APPLYCOLORS, m_bApplyColors);
	//}}AFX_DATA_MAP
}
...
BEGIN_MESSAGE_MAP(CSkinPropertyPageThemes, CPropertyPage)
	...
	ON_LBN_SELCHANGE(IDC_SKIN_LIST_THEMES, OnSelChangeListThemes)
	ON_CBN_SELCHANGE(IDC_SKIN_COMBO_COLORS, OnSelChangeComboColors)
	ON_CBN_SELCHANGE(IDC_SKIN_COMBO_FONTS, OnSelChangeComboFonts)
	//AFX_MSG_MAP
	ON_BN_CLICKED(IDC_SKIN_RADIO_OFFICE2007, OnRadioSchema)
	ON_BN_CLICKED(IDC_SKIN_RADIO_LUNA, OnRadioSchema)
	ON_BN_CLICKED(IDC_SKIN_RADIO_ROYALE, OnRadioSchema)
	ON_BN_CLICKED(IDC_SKIN_RADIO_VISTA, OnRadioSchema)
	ON_BN_CLICKED(IDC_SKIN_RADIO_DEFAULT, OnRadioSchema)
	ON_BN_CLICKED(IDC_SKIN_CHECK_APPLYMETRICS, OnCheckApplyMetrics)
	ON_BN_CLICKED(IDC_SKIN_CHECK_APPLYFRAME, OnCheckApplyFrame)
	ON_BN_CLICKED(IDC_SKIN_CHECK_APPLYCOLORS, OnCheckApplyColors)
	ON_BN_CLICKED(IDC_SKIN_RADIO_SKINLOCAL, OnReloadThemes)
	ON_BN_CLICKED(IDC_SKIN_RADIO_SKINSYSTEM, OnReloadThemes)
	ON_BN_CLICKED(IDC_SKIN_RADIO_OFFICE2000, OnThemeChanged)
	ON_BN_CLICKED(IDC_SKIN_RADIO_OFFICE2003, OnThemeChanged)
	ON_BN_CLICKED(IDC_SKIN_RADIO_OFFICEXP, OnThemeChanged)
	ON_BN_CLICKED(IDC_SKIN_RADIO_WINTHEME, OnThemeChanged)
	ON_BN_CLICKED(IDC_SKIN_RADIO_OFFICE2003_BLUE, OnThemeChangedLuna)
	ON_BN_CLICKED(IDC_SKIN_RADIO_OFFICE2003_OLIVE, OnThemeChangedLuna)
	ON_BN_CLICKED(IDC_SKIN_RADIO_OFFICE2003_SILVER, OnThemeChangedLuna)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
{
	UpdateData();
	((CMainFrame*)AfxGetMainWnd())->SchemaChanged(m_nSchema);
}
...
void CSkinPropertyPageThemes::FindThemes(CString strPath, BOOL bRecurse)
{
	...
	while(bNext)
	{
		...
		if (f.IsDirectory())
		{
			if (bRecurse)
			{
				FindThemes(f.GetFilePath() + _T("\\"), TRUE);
			}
		}
		else 
		{
...
void CSkinPropertyPageThemes::OnReloadThemes()
{
	ReloadThemes();
}
